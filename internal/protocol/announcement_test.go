...
func TestReadPeerAnnouncement_PeerJoined_RejectsZeroRole(t *testing.T) {
	frame := append([]byte{byte(protocol.AnnouncePeerJoined)}, bytes.Repeat([]byte{0xaa}, 32)...)
...
func TestReadPeerAnnouncement_AddressChanged_RejectsEmptyAddr(t *testing.T) {
	frame := append([]byte{byte(protocol.AnnounceAddressChanged)}, bytes.Repeat([]byte{0xaa}, 32)...)
...
func TestWritePeerAnnouncement_PropagatesAllWriteErrors(t *testing.T) {
	...
	for i, name := range []string{"kind", "pubkey", "role", "addr_len", "addr"} {
